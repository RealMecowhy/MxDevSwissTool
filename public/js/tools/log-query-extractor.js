...
function extractQueriesFromRecords(records) {
  ...
  for (const rec of records) {
    ...
    // XPath incoming
    let xpathMatch = msg.match(/^Incoming query of type (XPath|OQL):\s*\[([a-f0-9-]+)\]\s*(.*)/is);
    if (xpathMatch) {
    ...
    let oqlMatch = msg.match(/^OQL:\s*\[([a-f0-9-]+)\]\s*QueryParseResult\((.*)\)/is);
    if (oqlMatch) {
      const id = oqlMatch[1];
      let oqlContent = oqlMatch[2].trim();
      // Remove trailing Mendix metadata
      oqlContent = oqlContent.replace(/,com\.mendix\.connectionbus\..*$/s, '');
      if (!xpathMap.has(id)) xpathMap.set(id, { xpath: '', oql: '' });
  ...
  for (const rec of records) {
    ...
    let sqlMatch = msg.match(/^SQL@([a-f0-9]+)\((T\d+-C[a-f0-9]+)\):\s*(.*)/is);
    ...
    if (content.match(/^(Select|Update|Insert|Delete) params/i)) {
      ...
      let m = content.match(/^\[([a-f0-9-]+)\]\s*(.*)/is);
      q.xpathId = m[1];
...
function renderQueryList(list) {
  ...
  list.forEach((q, idx) => {
    ...
    el.innerHTML = `
      <div style="font-weight:600; color:${typeColor}">${q.type}</div>
