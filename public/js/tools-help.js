...
const TOOLS_HELP = {
  ...
  'error-decoder': {
    ...
    howToGet: `
      <ul>
        <li><strong>From a log:</strong> copy an <code>ERROR</code>/<code>CRITICAL</code> line and everything indented under it (the stack trace) from the Mendix log. In the <strong>Mendix Log Viewer</strong>, an ERROR row carries an <strong>Explain</strong> chip <em>only when the decoder recognizes its signature</em> — clicking it sends the full message here automatically. Rows the decoder cannot yet decode show no chip, so it never leads to a dead end.</li>
