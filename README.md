...
* **Log & Text Anonymizer**: Automatically strip sensitive PII (emails, IPs, UUIDs, MACs, Mendix object IDs, Luhn-checked card numbers, custom keywords and custom regexes) from logs before sharing them with support. **Secrets** are covered too: JWTs, `Bearer`/`Basic` values, AWS access key IDs, passwords embedded in URLs, `Cookie`/`Set-Cookie` values, and anything written as `api_key`/`client_secret`/`password = value` — including those same headers inside a **HAR**, where the name and value live in separate JSON fields. Only the secret itself is replaced: the label, header name, URL host and user stay readable, so the log is still diagnosable afterwards. Generic secrets are matched by their *label* rather than by value shape, because an API key has no universal format — so a secret stored under an unusual name needs a custom keyword or regex, which the Help states plainly.
